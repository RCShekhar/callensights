...
class AccountService(BaseService):
    ...
    def add_account(self, user_id: str, inputs: CreateAccountRequestModel) -> CreateAccountResponseModel:
        ...
        account_dict['job_submission_workflow'] = self.repository.get_workflow_id(workflow.value)
        account_dict['account_type'] = self.repository.get_internal_account_type_id(account_type)

        response = self.repository.add_account(account_dict)
        return CreateAccountResponseModel.model_validate(response)
