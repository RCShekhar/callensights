...
class User(Base):
    ...
    created_dt: Mapped[datetime] = mapped_column(
        "cu_created_dt", default=datetime.now()
    )
    ...
    user_group: Mapped[int] = mapped_column(
        "cu_user_group_id", ForeignKey("cns_user_group.ug_group_id"), nullable=False
    )
    manager_id: Mapped[int] = mapped_column(
        "cu_manager_id", ForeignKey("cns_user_def.cu_user_id"), nullable=True
    )
...
class Media(Base):
    ...
    user_id: Mapped[int] = mapped_column(
        "cm_user_id", ForeignKey("cns_user_def.cu_user_id"), nullable=False
    )
    ...
    is_uploaded: Mapped[bool] = mapped_column(
        "cm_is_uploaded", nullable=False, default=False
    )
...
class MediaStatus(Base):
    ...
    id: Mapped[int] = mapped_column(
        "ms_status_id", primary_key=True, autoincrement="auto"
    )
    # media_id # Put reference here
    media_id: Mapped[int] = mapped_column(
        "ms_media_id", ForeignKey("cns_media_def.cm_media_id"), nullable=False
    )
...
class ApplicationConfig(Base):
    ...
    created_dt: Mapped[datetime] = mapped_column(
        "ac_created_dt", default=datetime.now()
    )
...
class GroupMessages(Base):
    ...
    group_id: Mapped[int] = mapped_column(
        "gm_group_id", ForeignKey("UserGroup.id"), nullable=False
    )
    ...
    created_dt: Mapped[datetime] = mapped_column(
        "gm_created_dt", default=datetime.now()
    )
...
class LeadStages(Base):
    ...
    created_dt: Mapped[datetime] = mapped_column(
        "ls_created_dt", default=datetime.now()
    )
...
class LeadCallMetrics(Base):
    ...
    lead_type_id: Mapped[int] = mapped_column(
        "lcm_lead_type_id", ForeignKey("cns_lead_types_def.lt_type_id"), nullable=False
    )
    call_type_id: Mapped[int] = mapped_column(
        "lcm_call_type_id", ForeignKey("cns_lead_stage_def.ls_stage_id"), nullable=False
    )
    metrics_id: Mapped[int] = mapped_column(
        "lcm_metrics_id", ForeignKey("cns_metrics_def.md_metric_id"), nullable=False
    )

    lcm_created_dt: Mapped[datetime] = mapped_column(
        "lcm_created_dt", default=datetime.now()
    )
...
class Lead(Base):
    ...
    stage_id: Mapped[int] = mapped_column(
        "cl_stage_id", ForeignKey("cns_lead_stage_def.ls_stage_id"), nullable=True
    )
    assigned_to: Mapped[int] = mapped_column(
        "cl_assigned_to", ForeignKey("cns_user_def.cu_user_id"), nullable=True
    )
    lead_desc: Mapped[str] = mapped_column("cl_lead_desc")
    created_dt: Mapped[datetime] = mapped_column(
        "cl_created_dt", default=datetime.now()
    )
...
class LeadTypes(Base):
    ...
    created_dt: Mapped[datetime] = mapped_column(
        "lt_created_dt", default=datetime.now()
    )
...
class Metrics(Base):
    ...
    created_dt: Mapped[datetime] = mapped_column(
        "md_created_dt", default=datetime.now()
    )
...
class Activity(Base):
    ...
    done_by: Mapped[int] = mapped_column(
        "ca_done_by", ForeignKey("cns_user_def.cu_user_id"), nullable=False
    )
    lead_id: Mapped[int] = mapped_column(
        "ca_lead_id", ForeignKey("cns_lead_def.cl_lead_id"), nullable=True
    )
    ...
    affected_user: Mapped[int] = mapped_column(
        "ca_affected_user", ForeignKey("cns_user_def.cu_user_id"), nullable=True
    )
    event_date: Mapped[datetime] = mapped_column(
        "ca_activity_date", default=datetime.now()
    )
    stage_id: Mapped[int] = mapped_column(
        "ca_stage_id", ForeignKey("cns_lead_stage_def.ls_stage_id"), nullable=True
    )
    media_code: Mapped[str] = mapped_column(
        "ca_media_code", ForeignKey("cns_media_def.cm_media_code"), nullable=True
    )
