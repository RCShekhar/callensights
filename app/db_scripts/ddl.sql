...
CREATE VIEW callensights.metrics_view AS
select `cmd2`.`cm_media_code`   AS `media_code`,
       `cltd`.`lt_type_desc`    AS `lead_type_desc`,
       `clsd`.`ls_stage_desc`   AS `stage_desc`,
       `cld`.`cl_lead_name`     AS `lead_name`,
       `cmd`.`md_metric_prompt` AS `metric_prompt`,
       `clcm`.`lcm_key_metric`  AS `key_metric_flag`
from (((((`callensights`.`cns_lead_call_metrics` `clcm` join `callensights`.`cns_metrics_def` `cmd`
          on ((`cmd`.`md_metric_id` = `clcm`.`lcm_metrics_id`))) join `callensights`.`cns_lead_types_def` `cltd`
         on ((`clcm`.`lcm_lead_type_id` = `cltd`.`lt_type_id`))) join `callensights`.`cns_lead_stage_def` `clsd`
        on ((`clsd`.`ls_stage_id` = `clcm`.`lcm_call_type_id`))) join `callensights`.`cns_lead_def` `cld`
       on ((`cld`.`cl_stage_id` = `clsd`.`ls_stage_id`))) join `callensights`.`cns_media_def` `cmd2`
      on ((`cmd2`.`cm_lead_id` = `cld`.`cl_lead_id`)));


CREATE TRIGGER callensights.trg_media_status
    AFTER INSERT
    ON cns_media_def
    FOR EACH ROW
BEGIN
...
CREATE TRIGGER callensights.trg_status_update
    BEFORE UPDATE
    ON cns_media_status
    FOR EACH ROW
BEGIN
...
END IF;
IF
NEW.ms_fedbk_status_cd = 'R' THEN
        SET NEW.ms_fedbk_start_dt = CURRENT_TIMESTAMP();
    ELSEIF
NEW.ms_fedbk_status_cd IN ('C', 'S', 'E') THEN
...
ALTER TABLE cns_ats.cns_accounts
    ADD CONSTRAINT fk_cns_accounts_type FOREIGN KEY (account_type) REFERENCES cns_ats.cns_account_type (type_id) ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE cns_ats.cns_accounts
    ADD CONSTRAINT fk_cns_accounts_workflow FOREIGN KEY (job_submission_workflow) REFERENCES cns_ats.cns_job_submission_workflow (workflow_id) ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE cns_ats.cns_accounts
    ADD CONSTRAINT fk_cns_accounts_created FOREIGN KEY (created_by) REFERENCES cns_ats.cns_user_def (cu_user_id) ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE cns_ats.cns_accounts
    ADD CONSTRAINT fk_cns_accounts_modified FOREIGN KEY (modified_by) REFERENCES cns_ats.cns_user_def (cu_user_id) ON DELETE NO ACTION ON UPDATE NO ACTION;
